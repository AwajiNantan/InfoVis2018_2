function main()
{
    ...
    for ( var i = 0; i < 256; i++ )
    {
        var S = i/255.0; // [0.1,0.8]
        var R = Math.max( -1*Math.cos( (10/7)*( S - 0.1 ) * Math.PI ), 0.0 );
        var G = Math.max( Math.cos( (10/7)*( S -0.45 ) * Math.PI ), 0.0 );
        var B = Math.max( Math.cos( (10/7)*(S-0.1) * Math.PI ), 0.0 );
